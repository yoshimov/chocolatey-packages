#try { #error handling is only necessary if you need to do anything in addition to/instead of the main helpers
  ...
  Install-ChocolateyPackage 'freemind' 'EXE' '/S' 'http://downloads.sourceforge.net/project/freemind/freemind/0.9.0/FreeMind-Windows-Installer-0.9.0-max.exe?r=http%3A%2F%2Fsourceforge.net%2Fprojects%2Ffreemind%2Ffiles%2Ffreemind%2F0.9.0%2F&ts=1348108068&use_mirror=auto' -validExitCodes @(0)
  ...
  #Install-ChocolateyZipPackage 'freemind' 'URL' "$(Split-Path -parent $MyInvocation.MyCommand.Definition)" '64BIT_URL_DELETE_IF_NO_64BIT' 
  
  # other helpers - using any of these means you want to uncomment the error handling up top and at bottom.
  # downloader that the main helpers use to download items
  #Get-ChocolateyWebFile 'freemind' 'DOWNLOAD_TO_FILE_FULL_PATH' 'URL' '64BIT_URL_DELETE_IF_NO_64BIT'
  # installer, will assert administrative rights - used by Install-ChocolateyPackage
  #Install-ChocolateyInstallPackage 'freemind' 'EXE_OR_MSI' 'SILENT_ARGS' '_FULLFILEPATH_' -validExitCodes @(0)
  ...
  #$target = Join-Path $MyInvocation.MyCommand.Definition 'freemind.exe'
