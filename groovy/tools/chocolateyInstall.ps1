#try { #error handling is only necessary if you need to do anything in addition to/instead of the main helpers
  ...
  #Install-ChocolateyPackage 'groovy' 'EXE_OR_MSI' 'SILENT_ARGS' 'URL' '64BIT_URL_DELETE_IF_NO_64BIT'  -validExitCodes @(0)
  ...
  Install-ChocolateyZipPackage 'groovy' 'http://dist.groovy.codehaus.org/distributions/groovy-binary-2.0.2.zip' "$parentpath"
  
  # other helpers - using any of these means you want to uncomment the error handling up top and at bottom.
  # downloader that the main helpers use to download items
  #Get-ChocolateyWebFile 'groovy' 'DOWNLOAD_TO_FILE_FULL_PATH' 'URL' '64BIT_URL_DELETE_IF_NO_64BIT'
  # installer, will assert administrative rights - used by Install-ChocolateyPackage
  #Install-ChocolateyInstallPackage 'groovy' 'EXE_OR_MSI' 'SILENT_ARGS' '_FULLFILEPATH_' -validExitCodes @(0)
  ...
  Install-ChocolateyPath "$parentpath\groovy-2.0.2\bin" 'User' # Machine will assert administrative rights
  # add specific files as shortcuts to the desktop
  #$target = Join-Path $MyInvocation.MyCommand.Definition 'groovy.exe'
