#try { #error handling is only necessary if you need to do anything in addition to/instead of the main helpers
  ...
  #Install-ChocolateyPackage 'scala' 'EXE_OR_MSI' 'SILENT_ARGS' 'URL' '64BIT_URL_DELETE_IF_NO_64BIT'  -validExitCodes @(0)
  ...
  Install-ChocolateyZipPackage 'scala' 'http://www.scala-lang.org/downloads/distrib/files/scala-2.9.2.zip' "$parentpath" 
  
  # other helpers - using any of these means you want to uncomment the error handling up top and at bottom.
  # downloader that the main helpers use to download items
  #Get-ChocolateyWebFile 'scala' 'DOWNLOAD_TO_FILE_FULL_PATH' 'URL' '64BIT_URL_DELETE_IF_NO_64BIT'
  # installer, will assert administrative rights - used by Install-ChocolateyPackage
  #Install-ChocolateyInstallPackage 'scala' 'EXE_OR_MSI' 'SILENT_ARGS' '_FULLFILEPATH_' -validExitCodes @(0)
  ...
  Install-ChocolateyPath "$parentpath\scala-2.9.2\bin" 'User' # Machine will assert administrative rights
  # add specific files as shortcuts to the desktop
  #$target = Join-Path $MyInvocation.MyCommand.Definition 'scala.exe'
